...
const AdminDashboard = () => {
  ...
  const fetchModules = (courseId: string) => {
    axios.get(`http://localhost:3001/modules/${courseId}`).then(res => setModules(res.data));
  };

  const handleAddCourse = async () => {
    await axios.post('http://localhost:3001/courses', courseForm);
    alert('Course added!');
  ...
  const handleAddModule = async () => {
    await axios.post('http://localhost:3001/modules', moduleForm);
    alert('Module added!');
    setModuleForm({ title: '', course_id: '' });
  };

  const handleAddVideo = async () => {
    await axios.post('http://localhost:3001/videos', videoForm);
    alert('Video added!');
